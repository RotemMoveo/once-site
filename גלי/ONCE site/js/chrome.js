...
(function () {
  ...
  if (firstRow) {
    ...
    wrap.style.cssText = 'display:flex;flex-direction:row-reverse;flex-wrap:wrap;justify-content:flex-start;gap:80px 40px;width:1188px;max-width:100%;flex-shrink:0';
  ...
  function applyFilter(label) {
    cards.forEach(function (c) {
      ...
      var show = label === 'הכל' || label.indexOf(st.replace('מאוכלס', 'מאוכלס')) !== -1 || label.indexOf(st) !== -1 ||
...
(function () {
  ...
  box.innerHTML =
    ...
    '<button data-lb="close" style="position:absolute;top:28px;left:28px;background:none;border:none;cursor:pointer;padding:8px"><svg width="26" height="26" viewBox="0 0 22 22" fill="none"><path d="M2 2L20 20M20 2L2 20" stroke="white" stroke-width="1.5" stroke-linecap="round"/></svg></button>' +
